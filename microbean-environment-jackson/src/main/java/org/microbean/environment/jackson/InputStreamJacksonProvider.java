 ...
 */
package org.microbean.environment.jackson;
...
import org.microbean.environment.provider.CachingSupplier;

import org.microbean.environment.api.Loader;
import org.microbean.environment.api.Path;

import org.microbean.environment.jackson.JacksonProvider;
...
public class InputStreamJacksonProvider<T> extends JacksonProvider<T> {
  ...
  public InputStreamJacksonProvider(final Supplier<? extends ObjectMapper> mapperSupplier, final String resourceName) {
    this(objectCodecFunction(mapperSupplier),
         (l, p) -> inputStream(p.classLoader(), resourceName),
  ...
  protected final <T> ObjectCodec objectCodec(final Loader<?> requestingLoader, final Path<T> absolutePath) {
   ...
   */
  @Override // JacksonProvider<T>
  protected <T> TreeNode rootNode(final Loader<?> requestingLoader, final Path<T> absolutePath, final ObjectCodec objectCodec) {
